...
const openModal = (item, entity, onSuccess) => {
  ...
  if (isEditable) {
    modal.querySelector('.modal__btn--save').addEventListener('click', async () => {
      const updated = { ...item };
      modal.querySelectorAll('.modal__input').forEach(input => {
        updated[input.dataset.field] = input.value;
      });
      
      const id = item.osmId || item.poiId;
      const apiPath = entity === 'mapnodes' 
        ? `http://127.0.0.1:10001/api/mapnodes/${id}`
        : `http://127.0.0.1:10001/api/pois/${id}`;
      ...
      try {
        ...
        if (response.ok) {
          ...
          Notify.success('Изменения сохранены успешно!');
        } else {
          const errorData = await response.json().catch(() => ({}));
          Notify.error(`Ошибка при сохранении: ${response.status}`);
        }
      } catch (error) {
    ...
    modal.querySelector('.modal__btn--delete').addEventListener('click', async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json().catch(() => ({}));
          Notify.error(`Ошибка при удалении: ${response.status}`);
        }
      } catch (error) {
        Notify.error('Ошибка подключения к серверу');
      }
  ...
  tbody.querySelectorAll('tr[data-index]').forEach(row => {
    row.addEventListener('click', (e) => {
      ...
      openModal(tableState.items[index], tableState.entity, () => {
          fetchEntityData(entity).then((result) => {
